...
{
    public class UniversalEntryRepository : IRepository<IEntry>, IDisposable
    {
        private PhoneDataContext _context;
        private static object _lock = new object();

        public UniversalEntryRepository()
        {
            ...
            if (!string.IsNullOrWhiteSpace(entry.EntryText))
            {
                lock (_lock)
                {
                    ...
                    if (pEntry != null)
                    {
                        ...
                        pEntry.EditDateTime = DateTimeOffset.Now;
                        pEntry.MealType = entry.MealType;
                    }
                    else
                    {
                        pEntry = new UniversalEntry();
